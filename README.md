...
Compose scientific figures from SVG plots, PDF figures, standalone TeX figures, LaTeX equations, and raster images (PNG/JPEG) into a single SVG or PDF panel.
...
Define a panel template in Inkscape with named group (`<g>`) or rectangle (`<rect>`) placeholders (set via the layer/group label field), then write a small YAML config that maps each label to an SVG file, PDF, standalone `.tex` file, raster image (`.png`/`.jpg`), or a LaTeX string. `pc` scales each figure to fit its placeholder and writes the compiled panel. Raster images are embedded as-is at their pixel size and scaled to the placeholder, so a molecular render or photo can drop in directly:

```yaml
structure:
  file: laci_operator.png
  fit: contain
```
...
For LaTeX rendering and standalone `.tex` figures, `pdflatex` must be on your `PATH`. For PDF/TeX figures and PDF output, `pdf2svg` and `inkscape` must be on your `PATH`. On macOS: `brew install pdf2svg`. SVG and raster (PNG/JPEG) figures need no external tool.
